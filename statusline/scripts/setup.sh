...
if [[ "$SCRIPT_DIR" == *"/cache/"* ]]; then
    # Convert: /cache/marketplace-name/plugin-name/version-or-sha/scripts → /marketplaces/marketplace-name/plugin-name/scripts
    MARKETPLACE_PATH="${SCRIPT_DIR/\/cache\//\/marketplaces\/}"
    # Remove version (1.0.0) or commit SHA (hex string) directory from path
    MARKETPLACE_PATH=$(echo "$MARKETPLACE_PATH" | sed 's|/[0-9]\+\.[0-9]\+\.[0-9]\+/|/|; s|/[0-9a-f]\{6,40\}/|/|')
